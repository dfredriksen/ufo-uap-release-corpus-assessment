...
DEFAULT_VIDEO = SOURCE_ROOT / "DOD_111688825.mp4"
OUT_ROOT = Path("research/ufo-derived/video-motion-pass/pr27-d23-review") / VIDEO_ID
...
def phase_for(second: float) -> tuple[str, str]:
    if second < 116.0:
        return "pre-content", "DVIDS 00:00-01:55 no content"
    if second < 124.0:
        return "right-side acquisition", "DVIDS 01:56 area of contrast becomes distinguishable at center-right"
    if second < 134.0:
        return "sensor pans to center", "DVIDS 02:04 sensor pans to center on area of contrast"
    if second < 135.0:
        return "field-of-view narrows", "DVIDS 02:14 sensor field-of-view narrows to zoom"
    if second < 207.0:
        return "generally centered track", "DVIDS 02:15-03:26 area remains generally centered"
    return "erratic sensor motion and reacquisition", "DVIDS 03:27-04:57 sensor motion causes erratic display motion and repeated loss/reacquire"
...
def expected_center(second: float) -> tuple[float, float]:
    ...
    if second < 124.0:
        alpha = (second - 116.0) / 8.0
        return 1460.0 - 500.0 * alpha, 535.0
    return RETICLE_X, RETICLE_Y
...
def colored_overlay_mask(img: np.ndarray) -> np.ndarray:
    ...
    mask = ((sat > 45) & (val > 45) & (green | cyan | red_or_orange)).astype(np.uint8) * 255
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, np.ones((5, 5), np.uint8))
    return cv2.dilate(mask, np.ones((5, 5), np.uint8), iterations=1)


def nearest_overlay_distance(mask: np.ndarray, x: float, y: float) -> tuple[str, float | None]:
...
def remove_line_artifacts(mask: np.ndarray) -> np.ndarray:
    horizontal = cv2.morphologyEx(mask, cv2.MORPH_OPEN, np.ones((1, 29), np.uint8))
    vertical = cv2.morphologyEx(mask, cv2.MORPH_OPEN, np.ones((29, 1), np.uint8))
...
def candidate_mask(frame: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, float]:
    ...
    valid = overlay_clear & (gray > 24) & (gray < 245) & (hsv[:, :, 1] < 105)
    if int(valid.sum()) < 200:
        return roi, gray, np.zeros_like(gray, dtype=np.uint8), 0.0

    background = cv2.medianBlur(gray, 43)
    contrast = cv2.absdiff(gray, background)
    threshold = max(13.0, float(np.percentile(contrast[valid], 99.72)))
...
def detect_candidates(frame: np.ndarray, second: float, limit: int = 20) -> list[dict]:
    ...
    for label in range(1, labels_count):
        ...
        local_patch = gray[max(0, y - 22) : min(gray.shape[0], y + h + 22), max(0, x - 22) : min(gray.shape[1], x + w + 22)]
        ...
        fill = area / max(1, w * h)
        relation, nearest = nearest_overlay_distance(overlay_mask, full_x, full_y)
        score = (
            contrast_delta * 4.2
            + min(area, 220) * 1.1
            + min(fill, 0.65) * 80.0
            - dist_expected * 0.22
            - max(0.0, aspect - 3.0) * 16.0
        )
...
def classify(candidate: dict | None, previous_supported: dict | None, second: float) -> tuple[str, str, float | None]:
    if candidate is None:
        return "none", "no contrast component survived filtering", None
    ...
    if previous_supported is not None:
        step = math.hypot(float(candidate["center_x"]) - float(previous_supported["center_x"]), float(candidate["center_y"]) - float(previous_supported["center_y"]))

    if second < 116.0:
        return "low", "pre-content row; excluded from object support", step
    if contrast >= 36 and area >= 8 and width >= 2 and height >= 2 and distance <= 260 and not relation.startswith("intersects"):
        return "high", "compact contrast candidate aligned with PR27 phase description", step
    if contrast >= 24 and area >= 4 and distance <= 420 and not (relation.startswith("intersects") and area < 20):
        return "medium", "usable but weak, reacquisition-phase, or background-confounded contrast candidate", step
    return "low", "weak, distant from expected lane, overlay-adjacent, or background-confounded contrast candidate", step
...
def annotate_frame(frame: np.ndarray, candidate: dict | None, second: float, quality: str, phase: str) -> np.ndarray:
    ...
    cv2.drawMarker(out, (int(round(ex)), int(round(ey))), (255, 0, 0), markerType=cv2.MARKER_CROSS, markerSize=30, thickness=1)
    ...
    return add_label(out, f"{VIDEO_ID} t={second:05.1f}s PR27/D23 {phase}; quality={quality}")


def crop_patch(frame: np.ndarray, candidate: dict | None, second: float, size: int = 240) -> np.ndarray:
    h, w = frame.shape[:2]
    if candidate is not None:
...
def annotate_patch(frame: np.ndarray, candidate: dict | None, second: float, quality: str) -> np.ndarray:
    ...
    cv2.drawMarker(out, (240, 240), (0, 0, 255), markerType=cv2.MARKER_CROSS, markerSize=38, thickness=2)
    cv2.circle(out, (240, 240), 52, (0, 0, 255), 2)
...
def supported_interval_summary(rows: list[dict]) -> dict:
    ...
    return {"metric": "supported_intervals", "value": "; ".join(parts), "note": f"{len(ranges)} contiguous supported interval(s)"}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PR27/D23 one-fps video review for DOD_111688825.")
    parser.add_argument("--video", type=Path, default=DEFAULT_VIDEO)
    parser.add_argument("--sample-rate", type=float, default=1.0)
    parser.add_argument("--start-second", type=float, default=116.0)
    ...
    end_second = min(duration, args.end_second if args.end_second is not None else duration)

    seconds = []
    sample_count = int(math.floor((end_second - args.start_second) * args.sample_rate)) + 1
    for sample_index in range(sample_count):
        second = round(args.start_second + sample_index / args.sample_rate, 3)
    ...
    annotated_sheets = write_contact_sheets(annotated_paths, sheet_dir, f"{VIDEO_ID}-pr27-d23-annotated", cols=5, thumb_width=384)
    patch_sheets = write_contact_sheets(patch_paths, sheet_dir, f"{VIDEO_ID}-pr27-d23-patches", cols=8, thumb_width=180)

    review_csv = Path("research/ufo-video-pr27-d23-review-dod111688825.csv")
    write_csv(review_csv, list(rows[0].keys()), rows)

    supported = supported_rows(rows)
    quality_counts = Counter(row["review_quality"] for row in rows)
    phase_counts = Counter(row["dvids_phase"] for row in rows)
    ...
    expected_offsets = [float(row["distance_from_expected_px"]) for row in supported]
    ...
    summary_rows: list[dict] = [
        ...
        {"metric": "report_id", "value": REPORT_ID, "note": "DVIDS accompanying mission report"},
        {"metric": "duration_seconds", "value": round(duration, 3), "note": f"fps={fps}; frames={total_frames}"},
        {"metric": "sample_count", "value": len(rows), "note": f"{args.sample_rate} fps review samples from {args.start_second}s to {end_second:.3f}s"},
        ...
        track_stats(rows),
        numeric_summary("component_area", areas, "tracked contrast-component pixel area"),
        numeric_summary("contrast_delta_luma", contrasts, "max bright/dark local contrast delta"),
        numeric_summary("distance_from_expected_px", expected_offsets, "distance from DVIDS-phase expected lane"),
    ...
    for key, value in sorted(phase_counts.items()):
        summary_rows.append({"metric": f"phase_count: {key}", "value": value, "note": "all sampled rows by DVIDS phase"})
    ...
    for key, value in sorted(polarity_counts.items()):
        summary_rows.append({"metric": f"polarity_count: {key}", "value": value, "note": "supported candidate polarity"})
    ...
    summary_csv = Path("research/ufo-video-pr27-d23-review-summary.csv")
    write_csv(summary_csv, ["metric", "value", "note"], summary_rows)

    asset_rows: list[dict] = [
        {"artifact_type": "review_csv", "path": str(review_csv).replace("\\", "/"), "note": "PR27/D23 one-fps video-review table"},
        {"artifact_type": "summary_csv", "path": str(summary_csv).replace("\\", "/"), "note": "PR27/D23 review summary"},
    ]
    for path in annotated_sheets:
        asset_rows.append({"artifact_type": "annotated_contact_sheet", "path": str(path).replace("\\", "/"), "note": "PR27/D23 annotated frame sheet"})
    for path in patch_sheets:
        asset_rows.append({"artifact_type": "zoom_patch_contact_sheet", "path": str(path).replace("\\", "/"), "note": "PR27/D23 target patch sheet"})
    asset_csv = Path("research/ufo-video-pr27-d23-review-assets.csv")
