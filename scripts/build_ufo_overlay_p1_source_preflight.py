...
DEFAULT_OUTPUT = DOCS / "ufo-overlay-measurement-p1-source-preflight.csv"
DEFAULT_SUMMARY = DOCS / "ufo-overlay-measurement-p1-source-preflight.md"
...
USER_AGENT = "ufo-uap-release-corpus-assessment/overlay-preflight (+https://github.com/dfredriksen/ufo-uap-release-corpus-assessment)"
...
def head_url(url: str, timeout: int) -> tuple[str, str, str]:
    ...
    try:
        with urllib.request.urlopen(request, timeout=timeout) as response:
            return f"http_{response.status}", response.headers.get("Content-Length", ""), response.headers.get("Content-Type", "")
    except urllib.error.HTTPError as exc:
...
def extract_page_metadata(page: str) -> dict[str, str]:
    ...
    return {
        ...
        "duration": first_match(description, r"Video Duration:\s*([0-9:]+)") or first_match(page, r"Video Duration:\s*([0-9:]+)"),
...
def build_rows(residual_plan: Path, timeout: int) -> list[dict[str, str]]:
    ...
    for target in read_csv(residual_plan):
        ...
        is_p1_origin = clean(target.get("priority_band")) == "P1" or current_status.startswith("bounded_p1_review")
        if not is_p1_origin:
            continue
        ...
        acquisition_status = "embedded_public_mp4_found" if metadata["mp4"] else "no_embedded_public_mp4_found"
        ...
        if current_status.startswith("bounded_p1_review"):
            next_action = (
                "Source has since been acquired outside the repo and bounded-reviewed; run exhaustive all-frame OCR only if a stronger absence claim is needed."
            )
        else:
            next_action = "If acquired, store MP4 outside the repo, hash it, then run interval/crop review before classification."
        rows.append(
            {
...
def write_summary(path: Path, rows: list[dict[str, str]]) -> None:
    ...
    lines = [
        ...
        "## Summary",
        "",
        f"- P1-origin targets checked: `{len(rows)}`",
    ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Preflight P1 residual DVIDS source pages for embedded public MP4 URLs.")
...
def main() -> int:
    ...
    print(f"Wrote {len(rows)} P1 preflight rows to {args.output}")
