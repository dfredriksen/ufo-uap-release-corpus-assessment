...
DEFAULT_OUTPUT = DOCS / "ufo-overlay-measurement-residual-source-preflight.csv"
DEFAULT_SUMMARY = DOCS / "ufo-overlay-measurement-residual-source-preflight.md"
...
USER_AGENT = "ufo-uap-release-corpus-assessment/overlay-residual-preflight (+https://github.com/dfredriksen/ufo-uap-release-corpus-assessment)"
...
def build_rows(residual_plan: Path, timeout: int) -> list[dict[str, str]]:
    ...
    for target in read_csv(residual_plan):
        lane_status = clean(target.get("current_lane_status"))
        if lane_status not in {"metadata_only_unclassified", "source_preflighted_not_acquired"} and not lane_status.startswith(
            "bounded_residual_remote_ocr"
        ):
            continue
        if clean(target.get("local_source_hint")):
            continue
        if not clean(target.get("dvids_url")):
            continue
    ...
    rows.sort(key=lambda row: (row["release_tag"], row["record_id"], row["residual_id"]))
    return rows
...
def write_summary(path: Path, rows: list[dict[str, str]]) -> None:
    ...
    lines = [
        ...
        "## Summary",
        "",
        f"- Residual source pages checked: `{len(rows)}`",
        f"- Release 01 rows: `{by_release.get('release-01', 0)}`",
        f"- Release 02 rows: `{by_release.get('release-02', 0)}`",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Preflight remaining residual DVIDS source pages for embedded public MP4 URLs.")
...
def main() -> int:
    ...
    print(f"Wrote {len(rows)} residual preflight rows to {args.output}")
