...
def p2_local_targets(residual_plan: Path) -> list[dict[str, str]]:
    rows = []
    for row in read_csv(residual_plan):
        if clean(row.get("priority_band")) != "P2":
            continue
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a bounded OCR probe over local-source P2 overlay targets.")
...
def main() -> int:
    ...
    rows: list[dict[str, str]] = []
    for target in p2_local_targets(args.residual_plan):
